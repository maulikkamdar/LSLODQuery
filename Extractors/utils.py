...
class SPARQLOperator(object):
    ...
    def __init__(self, config_filename=None, query_filename=None):
        if config_filename:
            ...
            self.prefix_header = \
                self.set_prefix_header(self.config_details['prefixes'])
        if query_filename:
    ...
    def set_prefix_header(self, prefixes):
        ...
        for prefix in prefixes:
            prefix_header = prefix_header + 'PREFIX ' + prefix + ': <' \
                + prefixes[prefix] + '> '
...
def test_dictutils():
    ...
    du.iterative_merge_pickles('dat_files',
                               filename_generator=filename_gen,
                               exclusion_criteria=expcri,
                               final_name='assoc_paths_final.dat',
                               verbose=True)
