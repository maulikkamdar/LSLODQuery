...
def get_all_tag_list():
    ...
    for k in tag_list["result"]:
        ...
        for m in all_tag_s:
            if m in k: 
                if not is_ascii(k): continue
                ls_tag_list.append(k)
    return ls_tag_list
    
def ret_ckan_datasets(tag):
    url = data_hub_url + "package_search?fq=tags:" + urllib.quote(tag) + "&rows=500" #lifesciences
    print (url)
...
for k in int_dset:
    res_set = []
    for m in int_dset[k]["resources"]: res_set.append({"format": m["format"], "url": m["url"], "name": m["name"]})
    tag_set = []
    for m in int_dset[k]["tags"]: tag_set.append(m["name"])
    ref_set[k] = {"tags": tag_set, "resources": res_set, "title": int_dset[k]["title"], 
                  "url": int_dset[k]["url"], "extras": int_dset[k]["extras"], "notes": int_dset[k]["notes"]}
...
for k in ref_set:
    ...
    for m in ref_set[k]["resources"]:
        if m["format"] == "api/sparql" or "sparql" in m["url"]: spr = m["url"]
